...
break문을 만났을 경우에는 else구문이 실행되지 않음.
'''
for i in range(3):
    print(i, end=" ")
else:
...
# for문이 딱 한번만 실행된 후 탈출하므로 else는 실행되지 않는다.
for i in range(3):
    print(i, end=" ")
    break
else:
...
# 전체 누적 날짜수
total_days = 0
# 월별 날짜수를 리스티로 정의 (1월->31일, 7월8월->31일, 12월->31)
...
'''
입력한 월이 3이상이고 입력한 년도가 윤년이면 1을 더한다.
윤년은 2월이 29일까지 있기 때문.
'''
if month >= 3:
    if year % 400 == 0:
