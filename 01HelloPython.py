...
문장을 적당히 쓴 다음
블럭으로 설정한 후 싱글쿼테이션(')을 3개 쓰면 범위주석 가능!
'''
...
print("x / y", x / y)   # 나누기. 항상 float형 결과를 반환
print("x // y", x // y) # 나누기. 소수부분을 제거하므로 항상 int형의 결과를 반환
print("x * y", x * y)
print("x ** y", x ** y)         # 거듭제곱. #여기선 2의 4제곱을 계산한다.
print("pow(x, y)", pow(x, y))   # pow()함수를 통해 거듭제곱을 계산한다.
print("pow(x, y, z)", pow(x, y, z)) # x의 y제곱을 z로 나눈 나머지를 반환
print("divmod(x, y)", divmod(x, y)) # x와 y를 나눈 몫과 나머지를 튜플로 반환

# import는 모듈을 불러올때 사용하는 명령이다. #여기선 math모듈을 사용한다는 의미
import math
...
print(engStr[:3])   # 0~3까지의 범위에서 3앞까지만 가져온다. : Hel
...
# 인덱스 대신 변수를 사용하는 방법.
# 단 default값을 지정할 때는 "변수명=값" 으로 사용한다.
menu1 = "치킨"
menu2 = "맥주"
